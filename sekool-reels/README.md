...
| Ad | Length | Story |
|----|--------|-------|
| `class-at-home` | 15 s | "Tuition class. From the living room floor." Live 1 to 1 with a Cikgu Sekolah Kebangsaan, then the CTA |
| `report-card-day` | 12 s | Results slip (with a close-up of the marks) and the "No. 1 Dalam Kelas" plaque, then "Moments like this start at home." |
| `quick-cut` | 8 s | Five photos cut on the beat: Live classes / 1 to 1 / From home / Report card day / No. 1 Dalam Kelas, then the CTA |

All three end on a FREE Learning Roadmap call to action with sekool.my. The captions stay between y = 260 and y = 1250, so the Reels and Stories interface doesn't cover them.
...
`prepare_photos.py` blurs the school names and crests on the results slip and the plaque, and the child's full name on the plaque. The slip already had the name and IC number blurred. If you add new photos, check them for names, IC numbers and school names, and add boxes to `REDACT`.

## Before running `report-card-day` or the last two shots of `quick-cut`
...
python3 prepare_photos.py /path/to/originals   # expects 1.webp ... 5.webp
